      ...
      <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border/50">
        {!collapsed ? (
          <Link to="/" className={cn('flex items-center gap-2.5', isRTL && 'flex-row-reverse')}>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-role-business/20">
              <Briefcase className="w-5 h-5 text-role-business" />
            </div>
            <span className="font-bold text-lg text-role-business">Business AI</span>
          </Link>
        ) : (
          <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto bg-role-business/20">
            <Briefcase className="w-5 h-5 text-role-business" />
          </div>
        )}
      </div>
      ...
      <div className="p-3 border-t border-sidebar-border/50">
        {!collapsed && user && (
          <div className={cn(
            ...
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-lg bg-role-business/20">
              💼
            </div>
            <div className={cn('flex-1 min-w-0', isRTL && 'text-right')}>
              <p className="text-sm font-semibold truncate">{user.name}</p>
              <p className="text-xs capitalize font-medium text-role-business">Business Owner</p>
            </div>
          </div>
        )}
        ...
        {collapsed && user && (
          <Tooltip delayDuration={0}>
            <TooltipTrigger asChild>
              <div className="w-10 h-10 rounded-xl flex items-center justify-center mx-auto mb-2 text-lg cursor-default bg-role-business/20">
                💼
              </div>
            </TooltipTrigger>
            <TooltipContent side={isRTL ? 'left' : 'right'}>
              <p className="font-medium">{user.name}</p>
              <p className="text-xs text-muted-foreground capitalize">Business Owner</p>
            </TooltipContent>
          </Tooltip>
        )}
